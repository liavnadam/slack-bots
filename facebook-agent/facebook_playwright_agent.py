"""
Facebook Job Search Agent - Playwright Version

This version uses Playwright for browser automation instead of Graph API.
Works even when Graph API is blocked or unavailable.

Features:
- Real browser automation with Playwright
- Login with email/password
- Navigate Facebook groups
- Read posts and comments
- Post intelligent comments with job opportunities
- Smart rate limiting to avoid detection
- Persistent session/cookies
"""

import os
import time
import random
import json
from typing import List, Dict, Optional
from datetime import datetime
from dotenv import find_dotenv, load_dotenv
from playwright.sync_api import sync_playwright, Page, Browser, BrowserContext
from langchain.chat_models import ChatOpenAI
from langchain.prompts import ChatPromptTemplate
from langchain.chains import LLMChain
from job_opportunities import (
    JobOpportunity,
    JSONFileJobSource,
    JobMatcher
)

# Load environment variables
load_dotenv(find_dotenv())


class RateLimiter:
    """Smart rate limiter to avoid Facebook detection."""

    def __init__(
        self,
        min_delay: float = 5.0,
        max_delay: float = 15.0,
        posts_per_group: int = 2,
        max_posts_per_run: int = 5
    ):
        """
        Initialize rate limiter.

        Args:
            min_delay: Minimum delay between actions (seconds)
            max_delay: Maximum delay between actions (seconds)
            posts_per_group: Max comments per group per run
            max_posts_per_run: Total max comments per run
        """
        self.min_delay = min_delay
        self.max_delay = max_delay
        self.posts_per_group = posts_per_group
        self.max_posts_per_run = max_posts_per_run
        self.comments_this_run = 0
        self.comments_per_group = {}

    def can_comment_in_group(self, group_id: str) -> bool:
        """Check if we can comment in this group."""
        group_count = self.comments_per_group.get(group_id, 0)
        return group_count < self.posts_per_group

    def can_comment_overall(self) -> bool:
        """Check if we can comment at all."""
        return self.comments_this_run < self.max_posts_per_run

    def record_comment(self, group_id: Optional[str] = None):
        """Record a comment was posted."""
        self.comments_this_run += 1
        if group_id:
            self.comments_per_group[group_id] = self.comments_per_group.get(group_id, 0) + 1

    def wait(self, action: str = "next action"):
        """Wait with random delay to appear more human."""
        delay = random.uniform(self.min_delay, self.max_delay)
        print(f"⏳ Waiting {delay:.1f} seconds before {action}...")
        time.sleep(delay)

    def short_wait(self):
        """Short random delay for page interactions."""
        delay = random.uniform(1.0, 3.0)
        time.sleep(delay)


class FacebookPlaywrightAgent:
    """Facebook agent using Playwright browser automation."""

    def __init__(
        self,
        email: Optional[str] = None,
        password: Optional[str] = None,
        openai_api_key: Optional[str] = None,
        jobs_file: str = "jobs.json",
        groups_file: str = "groups.json",
        dry_run: bool = True,
        include_job_opportunities: bool = True,
        headless: bool = False,
        min_delay: float = 5.0,
        max_delay: float = 15.0,
        posts_per_group: int = 2,
        max_posts_per_run: int = 5,
        save_session: bool = True,
        session_file: str = "fb_session.json"
    ):
        """
        Initialize the Facebook Playwright Agent.

        Args:
            email: Facebook email
            password: Facebook password
            openai_api_key: OpenAI API key
            jobs_file: Path to jobs JSON file
            groups_file: Path to groups JSON file
            dry_run: If True, only preview comments
            include_job_opportunities: Include job matches in comments
            headless: Run browser in headless mode
            min_delay: Minimum delay between comments (seconds)
            max_delay: Maximum delay between comments (seconds)
            posts_per_group: Max comments per group per run
            max_posts_per_run: Total max comments per run
            save_session: Save cookies/session for reuse
            session_file: Path to session file
        """
        self.email = email or os.getenv("FACEBOOK_EMAIL")
        self.password = password or os.getenv("FACEBOOK_PASSWORD")
        self.openai_api_key = openai_api_key or os.getenv("OPENAI_API_KEY")
        self.dry_run = dry_run
        self.include_job_opportunities = include_job_opportunities
        self.headless = headless
        self.save_session = save_session
        self.session_file = session_file

        if not self.email or not self.password:
            raise ValueError("Facebook email and password are required")

        if not self.openai_api_key:
            raise ValueError("OpenAI API key is required")

        # Initialize rate limiter
        self.rate_limiter = RateLimiter(
            min_delay=min_delay,
            max_delay=max_delay,
            posts_per_group=posts_per_group,
            max_posts_per_run=max_posts_per_run
        )

        # Load jobs
        job_source = JSONFileJobSource(jobs_file)
        self.jobs = job_source.fetch_jobs()
        self.job_matcher = JobMatcher(self.jobs) if self.jobs else None

        # Load groups configuration
        self.groups_config = self._load_groups_config(groups_file)

        print(f"✓ Loaded {len(self.jobs)} job opportunities")
        print(f"✓ Loaded {len(self.groups_config)} group configurations")

        # Initialize LangChain
        self.llm = ChatOpenAI(
            model_name="gpt-3.5-turbo",
            temperature=0.7,
            openai_api_key=self.openai_api_key
        )

        # Setup prompts
        self._setup_prompts()

        # Playwright objects (initialized in run)
        self.browser: Optional[Browser] = None
        self.context: Optional[BrowserContext] = None
        self.page: Optional[Page] = None

        # Job search keywords
        self.job_keywords = [
            "job search", "looking for work", "hiring", "job hunting",
            "need a job", "seeking employment", "open to work",
            "unemployed", "career change", "job opportunity",
            "resume tips", "interview", "job application",
            "looking for opportunities", "seeking a position",
            # Hebrew keywords
            "מחפש עבודה", "מחפשת עבודה", "דרוש", "דרושה", "משרה",
            "קורות חיים", "ראיון עבודה", "הזדמנות עבודה"
        ]

    def _setup_prompts(self):
        """Setup LangChain prompts."""
        # Prompt without job opportunity
        self.comment_prompt_no_job = ChatPromptTemplate.from_template(
            """You are a helpful career advisor providing supportive advice to job seekers.

A person posted this on Facebook about their job search:

"{post_content}"

Generate a brief, friendly, helpful comment (2-3 sentences) that:
- Shows empathy and encouragement
- Provides ONE specific, actionable tip
- Feels natural and human (not robotic or formal)
- Is specific to their situation
- Avoids being salesy
- If the post is in Hebrew, respond in Hebrew

Comment:"""
        )

        # Prompt with job opportunity
        self.comment_prompt_with_job = ChatPromptTemplate.from_template(
            """You are a helpful career advisor providing supportive advice to job seekers.

A person posted this on Facebook about their job search:

"{post_content}"

You have a relevant job opportunity to share:

Job Title: {job_title}
Location: {job_location}
Key Requirements: {job_requirements}
URL: {job_url}

Generate a brief, friendly comment (3-4 sentences) that:
- Shows empathy and encouragement
- Provides ONE actionable tip related to their search
- Naturally mentions you saw a relevant opportunity that might interest them
- Briefly highlights why it could be a good fit (1 sentence max)
- Includes the job URL
- Feels personal, not like a job board spam
- Doesn't sound overly promotional
- If the post is in Hebrew, respond in Hebrew

Comment:"""
        )

        self.comment_chain_no_job = LLMChain(
            llm=self.llm,
            prompt=self.comment_prompt_no_job,
            verbose=False
        )

        self.comment_chain_with_job = LLMChain(
            llm=self.llm,
            prompt=self.comment_prompt_with_job,
            verbose=False
        )

    def _load_groups_config(self, groups_file: str) -> List[Dict]:
        """Load groups configuration from JSON file."""
        if not os.path.exists(groups_file):
            print(f"Warning: Groups file not found at {groups_file}")
            return []

        with open(groups_file, 'r', encoding='utf-8') as f:
            return json.load(f)

    def is_job_related_post(self, post_text: str) -> bool:
        """Check if post is job-related."""
        if not post_text:
            return False

        post_lower = post_text.lower()
        return any(keyword.lower() in post_lower for keyword in self.job_keywords)

    def generate_comment(
        self,
        post_content: str,
        matched_job: Optional[JobOpportunity] = None
    ) -> str:
        """Generate a comment with optional job opportunity."""
        try:
            if matched_job and self.include_job_opportunities:
                # Generate comment with job opportunity
                comment = self.comment_chain_with_job.run(
                    post_content=post_content,
                    job_title=matched_job.title,
                    job_location=matched_job.location,
                    job_requirements=", ".join(matched_job.requirements[:3]),
                    job_url=matched_job.url
                )
            else:
                # Generate general supportive comment
                comment = self.comment_chain_no_job.run(
                    post_content=post_content
                )

            return comment.strip()
        except Exception as e:
            print(f"  ✗ Error generating comment: {e}")
            return ""

    def _save_session(self):
        """Save browser session/cookies."""
        if not self.save_session or not self.context:
            return

        try:
            cookies = self.context.cookies()
            storage_state = self.context.storage_state()

            with open(self.session_file, 'w') as f:
                json.dump(storage_state, f)

            print(f"✓ Session saved to {self.session_file}")
        except Exception as e:
            print(f"⚠️  Could not save session: {e}")

    def _load_session(self) -> Optional[Dict]:
        """Load saved browser session/cookies."""
        if not self.save_session or not os.path.exists(self.session_file):
            return None

        try:
            with open(self.session_file, 'r') as f:
                storage_state = json.load(f)
            print(f"✓ Loaded session from {self.session_file}")
            return storage_state
        except Exception as e:
            print(f"⚠️  Could not load session: {e}")
            return None

    def _login_to_facebook(self):
        """Login to Facebook using Playwright."""
        print("🔐 Logging in to Facebook...")

        self.page.goto("https://www.facebook.com/", wait_until="networkidle")
        self.rate_limiter.short_wait()

        try:
            # Check if already logged in
            if "login" not in self.page.url.lower():
                print("✓ Already logged in!")
                return True

            # Fill email
            email_input = self.page.locator('input[name="email"]')
            email_input.fill(self.email)
            self.rate_limiter.short_wait()

            # Fill password
            password_input = self.page.locator('input[name="pass"]')
            password_input.fill(self.password)
            self.rate_limiter.short_wait()

            # Click login button
            login_button = self.page.locator('button[name="login"]')
            login_button.click()

            # Wait for navigation
            self.page.wait_for_load_state("networkidle", timeout=30000)
            self.rate_limiter.short_wait()

            # Check if login successful
            if "login" in self.page.url.lower():
                print("❌ Login failed - check credentials or CAPTCHA")
                return False

            print("✓ Successfully logged in!")
            self._save_session()
            return True

        except Exception as e:
            print(f"❌ Login error: {e}")
            return False

    def _navigate_to_group(self, group_url: str):
        """Navigate to a Facebook group."""
        print(f"📍 Navigating to group: {group_url}")
        self.page.goto(group_url, wait_until="networkidle", timeout=60000)
        self.rate_limiter.short_wait()

        # Scroll to load more posts
        for _ in range(3):
            self.page.evaluate("window.scrollBy(0, 500)")
            time.sleep(1)

    def _scrape_group_posts(self, group_id: str, limit: int = 20) -> List[Dict]:
        """Scrape posts from a Facebook group."""
        group_url = f"https://www.facebook.com/groups/{group_id}"
        self._navigate_to_group(group_url)

        posts = []

        try:
            # Wait for posts to load
            self.page.wait_for_selector('[role="article"]', timeout=10000)

            # Get all post elements
            post_elements = self.page.locator('[role="article"]').all()

            print(f"  ✓ Found {len(post_elements)} posts on page")

            for idx, post_element in enumerate(post_elements[:limit]):
                try:
                    # Extract post text
                    text_content = ""
                    text_selectors = [
                        'div[dir="auto"]',
                        '[data-ad-preview="message"]',
                        '.userContent'
                    ]

                    for selector in text_selectors:
                        try:
                            text_elements = post_element.locator(selector).all()
                            if text_elements:
                                text_content = "\n".join([el.inner_text() for el in text_elements[:3]])
                                if text_content:
                                    break
                        except:
                            continue

                    if not text_content or len(text_content) < 10:
                        continue

                    # Store post info
                    post_data = {
                        'id': f"post_{idx}",
                        'message': text_content[:500],  # Limit length
                        'element': post_element,  # Keep reference for commenting
                        'group_id': group_id
                    }

                    posts.append(post_data)

                except Exception as e:
                    print(f"  ⚠️  Could not parse post {idx}: {e}")
                    continue

            print(f"  ✓ Successfully scraped {len(posts)} posts")

        except Exception as e:
            print(f"  ✗ Error scraping posts: {e}")

        return posts

    def _post_comment_on_element(self, post_element, comment_text: str) -> bool:
        """Post a comment on a post element."""
        if self.dry_run:
            print(f"\n  [DRY RUN] Would post:")
            print(f"  '{comment_text}'\n")
            return True

        try:
            # Find comment box (multiple selectors for different Facebook layouts)
            comment_selectors = [
                'div[aria-label="Write a comment..."]',
                'div[aria-label="כתוב תגובה..."]',
                '[role="textbox"]'
            ]

            comment_box = None
            for selector in comment_selectors:
                try:
                    comment_box = post_element.locator(selector).first
                    if comment_box.is_visible():
                        break
                except:
                    continue

            if not comment_box:
                print("  ✗ Could not find comment box")
                return False

            # Click comment box to activate
            comment_box.click()
            self.rate_limiter.short_wait()

            # Type comment
            comment_box.fill(comment_text)
            self.rate_limiter.short_wait()

            # Find and click submit button
            submit_selectors = [
                'div[aria-label="Comment"]',
                'div[aria-label="תגובה"]',
                '[type="submit"]'
            ]

            for selector in submit_selectors:
                try:
                    submit_button = post_element.locator(selector).first
                    if submit_button.is_visible():
                        submit_button.click()
                        print("  ✓ Successfully posted comment")
                        return True
                except:
                    continue

            print("  ✗ Could not find submit button")
            return False

        except Exception as e:
            print(f"  ✗ Error posting comment: {e}")
            return False

    def run_on_groups(self, city_filter: Optional[List[str]] = None):
        """
        Run agent on configured groups.

        Args:
            city_filter: Only run on groups in these cities (optional)
        """
        print("=" * 70)
        print("Facebook Job Search Agent - Playwright Edition")
        print("=" * 70)
        print(f"Mode: {'🔍 DRY RUN (preview only)' if self.dry_run else '🚀 LIVE'}")
        print(f"Browser: {'👻 Headless' if self.headless else '👀 Visible'}")
        print(f"Job Opportunities: {'✓ Enabled' if self.include_job_opportunities else '✗ Disabled'}")
        print(f"Rate Limit: {self.rate_limiter.posts_per_group} per group, {self.rate_limiter.max_posts_per_run} total")
        print(f"Delay: {self.rate_limiter.min_delay}s - {self.rate_limiter.max_delay}s between comments")
        print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 70)
        print()

        # Filter groups by city if specified
        target_groups = self.groups_config
        if city_filter:
            target_groups = [
                g for g in self.groups_config
                if g.get('city', '').lower() in [c.lower() for c in city_filter]
            ]
            print(f"🎯 Filtering to cities: {', '.join(city_filter)}")

        if not target_groups:
            print("❌ No groups to search. Check your groups.json configuration.")
            return

        print(f"📍 Targeting {len(target_groups)} group(s):\n")
        for group in target_groups:
            print(f"   • {group['name']} ({group['city']})")
        print()

        # Initialize Playwright
        with sync_playwright() as playwright:
            print("🚀 Launching browser...")

            # Launch browser
            self.browser = playwright.chromium.launch(headless=self.headless)

            # Load session if available
            storage_state = self._load_session()

            if storage_state:
                self.context = self.browser.new_context(storage_state=storage_state)
            else:
                self.context = self.browser.new_context()

            self.page = self.context.new_page()

            # Login
            if not self._login_to_facebook():
                print("❌ Login failed. Exiting.")
                self.browser.close()
                return

            print("\n" + "=" * 70)
            print("Starting group processing...")
            print("=" * 70 + "\n")

            total_processed = 0
            total_commented = 0

            # Process each group
            for group in target_groups:
                if not self.rate_limiter.can_comment_overall():
                    print(f"\n⚠️  Reached maximum comments per run ({self.rate_limiter.max_posts_per_run})")
                    break

                group_id = group['group_id']
                group_name = group['name']
                group_city = group.get('city', 'Unknown')

                print(f"\n{'─' * 70}")
                print(f"📍 Processing: {group_name} ({group_city})")
                print(f"{'─' * 70}")

                # Fetch posts
                posts = self._scrape_group_posts(group_id, limit=30)

                if not posts:
                    print(f"  ℹ️  No posts found or unable to access group")
                    continue

                # Filter job-related posts
                job_posts = [p for p in posts if self.is_job_related_post(p.get('message', ''))]
                print(f"  ✓ Identified {len(job_posts)} job-related posts")

                # Comment on posts
                group_comments = 0

                for post in job_posts:
                    if not self.rate_limiter.can_comment_in_group(group_id):
                        print(f"  ⚠️  Reached limit for this group ({self.rate_limiter.posts_per_group} comments)")
                        break

                    if not self.rate_limiter.can_comment_overall():
                        break

                    message = post.get('message', '')
                    post_element = post.get('element')

                    print(f"\n  📝 Post preview: {message[:100]}...")

                    # Try to match a job
                    matched_job = None
                    if self.job_matcher and self.include_job_opportunities:
                        matched_job = self.job_matcher.find_best_match(message, group_city)

                        if matched_job:
                            print(f"  ✨ Matched job: {matched_job.title} ({matched_job.city})")
                        else:
                            print(f"  ℹ️  No specific job match found")

                    # Generate comment
                    comment = self.generate_comment(message, matched_job)

                    if comment:
                        # Post comment
                        success = self._post_comment_on_element(post_element, comment)

                        if success:
                            self.rate_limiter.record_comment(group_id)
                            group_comments += 1
                            total_commented += 1

                            # Wait before next comment (unless last comment)
                            if self.rate_limiter.can_comment_overall():
                                self.rate_limiter.wait("next comment")

                    total_processed += 1

                print(f"\n  ✓ Posted {group_comments} comment(s) in this group")

                # Wait before next group
                if self.rate_limiter.can_comment_overall():
                    self.rate_limiter.wait("next group")

            # Save session before closing
            self._save_session()

            # Close browser
            print("\n🔒 Closing browser...")
            self.browser.close()

            # Final summary
            print(f"\n{'=' * 70}")
            print(f"✅ COMPLETED")
            print(f"{'=' * 70}")
            print(f"Posts processed: {total_processed}")
            print(f"Comments posted: {total_commented}")
            print(f"Groups processed: {len(target_groups)}")
            print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"{'=' * 70}\n")


def main():
    """Main function."""
    # Initialize agent
    agent = FacebookPlaywrightAgent(
        jobs_file="jobs.json",
        groups_file="groups.json",
        dry_run=True,  # Set to False for live posting
        include_job_opportunities=True,
        headless=False,  # Set to True to hide browser
        min_delay=5.0,
        max_delay=15.0,
        posts_per_group=2,
        max_posts_per_run=5,
        save_session=True
    )

    # Run on all configured groups
    agent.run_on_groups()

    # Or run only on specific cities
    # agent.run_on_groups(city_filter=["San Francisco", "New York"])


if __name__ == "__main__":
    main()
